...
// --- Helpers (Con cambios para Bs.S) ---
const formatMoney = (amount, currency = 'VES', locale = 'es-VE') => {
    if (amount === null || amount === undefined || isNaN(parseFloat(amount))) {
        return 'N/A';
    }
    const options = { style: 'currency', currency: currency, minimumFractionDigits: 2, maximumFractionDigits: 2, currencyDisplay: 'symbol' };
    if (currency === 'USD') {
        locale = 'en-US';
    }
    let formattedString = parseFloat(amount).toLocaleString(locale, options);
    // Reemplazo visual de VES a Bs.S
    if (currency === 'VES') {
        formattedString = formattedString.replace(/VES/g, 'Bs.S').replace(/\s/g, ' ');
    }
...
function RepresentativeStatementPage() {
    ...
    const displayTotalCharges = displayCurrency === 'VES' ? formatMoney(summary?.total_charges_ves_emission, 'VES') : formatMoney(summary?.total_due_original_currency_usd, 'USD');
    const displayTotalPayments = displayCurrency === 'VES' ? formatMoney(summary?.total_payments_received_ves, 'VES') : formatMoney(summary?.total_paid_original_currency_equivalent_usd, 'USD');
    const displayBalanceDue = displayCurrency === 'VES' ? formatMoney(summary?.current_balance_due_ves_today, 'VES') : formatMoney(summary?.current_balance_due_usd, 'USD');
    const displayAvailableCredit = displayCurrency === 'VES' ? formatMoney(summary?.explicit_available_credit_ves, 'VES') : formatMoney(summary?.explicit_available_credit_usd_equivalent, 'USD');
        ...
        <div className="bg-slate-50 min-h-screen p-4 sm:p-6 lg:p-8 font-sans">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="bg-white shadow-xl shadow-slate-200/60 rounded-xl p-6 mb-8">
                    <div className="flex flex-col sm:flex-row justify-between sm:items-center mb-5 border-b border-slate-200 pb-3">
                        ...
                        <button
                            ...
                            title={!currentUSDRate && displayCurrency === 'VES' ? "Tasa no disponible para cambiar vista" : `Ver en ${displayCurrency === 'VES' ? 'USD' : 'Bs.S'}`}
                        >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
                        <div className="bg-slate-100 p-4 rounded-lg"><p className="font-medium text-slate-600">Total Cargado ({displayCurrency === 'VES' ? 'Bs.S Emisión*' : 'USD Equiv.'})</p><p className="text-2xl font-bold text-slate-800">{displayTotalCharges}</p>{displayCurrency === 'VES' && <p className="text-xs text-slate-500 italic mt-1">*Total en Bs.S al momento de emisión.</p>}</div>
                        <div className="bg-slate-100 p-4 rounded-lg"><p className="font-medium text-slate-600">Total Pagos ({displayCurrency === 'VES' ? 'Bs.S' : 'USD'})</p><p className="text-2xl font-bold text-slate-800">{displayTotalPayments}</p></div>
                        <div className={`p-4 rounded-lg shadow-inner ${(summary?.current_balance_due_usd > 0.01) ? 'bg-red-100' : 'bg-green-100'}`}><p className={`font-semibold ${(summary?.current_balance_due_usd > 0.01) ? 'text-red-800' : 'text-green-800'}`}>Balance Adeudado ({displayCurrency === 'VES' ? 'Bs.S Indexado' : 'USD'})</p><p className={`text-3xl font-extrabold ${(summary?.current_balance_due_usd > 0.01) ? 'text-red-700' : 'text-green-700'}`}>{displayBalanceDue}</p></div>
                        <div className={`p-4 rounded-lg shadow-inner ${(hasCreditToApply) ? 'bg-cyan-50' : 'bg-slate-100'}`}><p className={`font-semibold ${(hasCreditToApply) ? 'text-cyan-800' : 'text-slate-600'}`}>Saldo a Favor ({displayCurrency === 'VES' ? 'Bs.S' : 'USD'})</p><p className={`text-2xl font-bold ${(hasCreditToApply) ? 'text-cyan-700' : 'text-slate-800'}`}>{displayAvailableCredit}</p></div>
                    </div>
                    <div className="mt-5 p-3 bg-amber-50 rounded-lg"><p className="text-sm font-semibold text-amber-800">Nota Importante:</p><p className="text-xs text-amber-700">"Balance Adeudado (Bs.S Indexado)" es la deuda pendiente convertida a Bs.S con la tasa de cambio más reciente, reflejando el monto a pagar hoy.</p></div>
                </div>
                        ...
                        <div className="overflow-x-auto">
                            <table className="min-w-full divide-y divide-slate-200 text-sm">
                                <thead className="bg-slate-50">
                                    <tr>
                                        {['Estudiante', 'Concepto', 'Emisión', 'Vencim.', 'Monto Orig.', 'Pagado Orig.', 'Deuda Orig.', 'Deuda Bs.S (Hoy)', 'Estado'].map(header => (
                    ...
                    {detailedPayments.length > 0 ? (
                        <div className="overflow-x-auto">
                            <table className="min-w-full divide-y divide-slate-200 text-sm">
                                <thead className="bg-slate-50">
                                    <tr>
                                       {['Fecha', 'Monto Pagado', 'Equiv. Bs.S (Día Pago)', 'Método', 'Referencia', 'Asignado (Bs.S)', 'Sobrante (Bs.S)', 'Acciones'].map(header => (
                ...
                <Modal isOpen={isApplyCreditModalOpen} onClose={() => setIsApplyCreditModalOpen(false)} title="Confirmar Aplicación de Saldo a Favor">
                    <div>
                        <p className="text-sm text-slate-600 mb-4">¿Está seguro de que desea aplicar el saldo a favor de <strong className="font-bold text-slate-800">{formatMoney(summary?.explicit_available_credit_ves, 'VES')}</strong> a las deudas pendientes de este representante?</p>
