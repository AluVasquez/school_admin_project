...
import {
    ...
    getPayrollRunById, // Para cargar detalles antes de confirmar o ver
    confirmPayrollRun,
    updatePayrollRunStatus,
    deletePayrollRunDraft,
    getEmployees // Para el modal de confirmación de nómina por horas
} from '../services/apiPersonnel';
import Modal from '../components/Modal';
import { Link }
    from 'react-router-dom'; // Opcional: para enlazar a una página de detalle de la corrida
...
const initialCreateFormData = {
    ...
    exchange_rate_usd_ves: '', // Opcional, el backend puede tomar el más reciente
};
...
function PayrollRunsPage() {
    ...
    // Modal para Confirmar Nómina (especialmente para horas)
    const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
    const [runToConfirm, setRunToConfirm] = useState(null); // PayrollRun object
    const [hourlyEmployeesForConfirm, setHourlyEmployeesForConfirm] = useState([]);
    const [employeeHoursInput, setEmployeeHoursInput] = useState({}); // { employeeId: hours }
    ...
    // --- Manejadores para Confirmar Nómina (con input de horas) ---
    const openConfirmModal = async (run) => {
        setRunToConfirm(run);
        setFormConfirmError(null);
        setEmployeeHoursInput({}); // Resetear horas
        if (run.pay_frequency_covered === 'hourly') {
            setIsLoadingConfirmData(true);
            try {
                // Cargar empleados activos con frecuencia por hora para esta corrida
                const params = { isActive: true, payFrequency: 'hourly', limit: 500 /* Ajustar si hay muchos */ };
                const empData = await getEmployees(token, params);
                setHourlyEmployeesForConfirm(empData.items || []);
                // Pre-llenar employeeHoursInput con horas acumuladas si existen
                const initialHours = {};
                (empData.items || []).forEach(emp => {
                    initialHours[emp.id.toString()] = emp.accumulated_hours > 0 ? emp.accumulated_hours.toString() : '0';
                });
                setEmployeeHoursInput(initialHours);

            } catch (err) {
                toast.error("Error cargando empleados por hora para confirmación.");
                setHourlyEmployeesForConfirm([]);
            } finally {
                setIsLoadingConfirmData(false);
            }
        } else {
            setHourlyEmployeesForConfirm([]); // No se necesitan empleados para otras frecuencias
        }
        setIsConfirmModalOpen(true);
    };
    const handleHourInputChange = (employeeId, hours) => {
        setEmployeeHoursInput(prev => ({ ...prev, [employeeId.toString()]: hours }));
    };
    const handleSubmitConfirmRun = async (e) => {
        ...
        try {
            ...
            if (runToConfirm.pay_frequency_covered === 'hourly' && hourlyEmployeesForConfirm.length > 0) {
                payload.employee_hours_input = Object.entries(employeeHoursInput)
                    .map(([empId, hoursStr]) => ({
                        employee_id: parseInt(empId),
                        hours: parseFloat(hoursStr) || 0,
                    }))
                    .filter(item => item.hours > 0); // Solo enviar empleados con horas > 0
            }
    ...
    const openStatusModal = (run) => {
        ...
        setStatusUpdateFormData({ status: run.status, notes: '' }); // Pre-seleccionar estado actual
        ...
        <div className="container mx-auto p-4">
                    ...
                    <div className="bg-white shadow-md rounded-lg overflow-x-auto">
                        <table className="min-w-full divide-y divide-gray-200 text-xs">
                            <thead className="bg-gray-100">
                                <tr>
                                    <th className="px-3 py-2 text-left font-semibold">Nombre Proceso</th>
                                    <th className="px-3 py-2 text-left font-semibold">Período</th>
                                    <th className="px-3 py-2 text-left font-semibold">Frecuencia</th>
                                    <th className="px-3 py-2 text-right font-semibold">Tasa USD (VES)</th>
                                ...
                                {payrollRuns.length > 0 ? payrollRuns.map(run => (
                                    <tr key={run.id} className="hover:bg-slate-200">
                                        ...
                                        <td className="px-3 py-2 text-right space-x-1">
                                            <Link to={`/personnel/payroll-runs/${run.id}/details`} className="text-blue-600 hover:text-blue-800">Ver</Link>
                                            {run.status === 'draft' && (<button onClick={() => openConfirmModal(run)} className="text-green-600 hover:text-green-800">Confirmar</button>)}
                                            {run.status !== 'cancelled' && (<button onClick={() => openStatusModal(run)} className="text-gray-600 hover:text-gray-800">Estado</button>)}
            ...
            <Modal isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)} title="Iniciar Nuevo Proceso de Nómina (Borrador)">
                <form onSubmit={handleSubmitCreateDraft} className="space-y-4 text-sm">
                    ...
                    <div><label htmlFor="exchange_rate_usd_ves_pr_create" className="block font-medium">Tasa USD-VES (Opcional)</label><input type="number" name="exchange_rate_usd_ves" id="exchange_rate_usd_ves_pr_create" value={createFormData.exchange_rate_usd_ves} onChange={handleCreateInputChange} step="any" placeholder="Si se omite, se usará la más reciente" className="mt-1 input-style"/><p className="text-xs text-gray-500">Dejar vacío para usar la tasa más reciente del sistema para la fecha fin del período.</p></div>
                    {formCreateError && <p className="text-red-500 text-xs p-2 bg-red-50 rounded">{formCreateError}</p>}
                    <div className="pt-4 flex justify-end space-x-2 border-t"><button type="button" onClick={()=>setIsCreateModalOpen(false)} className="btn-secondary-xs px-3 py-1.5">Cancelar</button><button type="submit" disabled={isSubmittingCreate} className="btn-primary-xs px-3 py-1.5 disabled:opacity-50">{isSubmittingCreate ? 'Creando...' : 'Crear Borrador'}</button></div>
                </form>
            </Modal>

            {/* Modal para Confirmar Nómina (con input de horas si es necesario) */}
            {runToConfirm && (
            <Modal isOpen={isConfirmModalOpen} onClose={() => setIsConfirmModalOpen(false)} title={`Confirmar Nómina: "${runToConfirm.name}"`}>
                <form onSubmit={handleSubmitConfirmRun} className="space-y-4 text-sm">
                    <p className="text-gray-700">Está a punto de confirmar y aplicar los cálculos de esta nómina a los saldos de los empleados. Esta acción es importante.</p>
                    ...
                    <p className="text-xs">Tasa USD (si aplica): <span className="font-medium">{runToConfirm.exchange_rate_usd_ves ? parseFloat(runToConfirm.exchange_rate_usd_ves).toFixed(2) + ' VES' : 'Se usará la más reciente'}</span></p>

                    {runToConfirm.pay_frequency_covered === 'hourly' && (
                        <div className="mt-4 pt-3 border-t">
                            <h4 className="font-medium mb-2 text-gray-700">Ingresar Horas para Empleados por Hora:</h4>
                            {isLoadingConfirmData && <p>Cargando empleados por hora...</p>}
                            {!isLoadingConfirmData && hourlyEmployeesForConfirm.length === 0 && <p className="text-gray-500 text-xs">No hay empleados por hora activos para esta frecuencia o ya no tienen horas acumuladas.</p>}
                            {!isLoadingConfirmData && hourlyEmployeesForConfirm.map(emp => (
                                <div key={emp.id} className="grid grid-cols-2 gap-2 items-center mb-1.5">
                                    <label htmlFor={`hours_emp_${emp.id}`} className="text-xs truncate" title={emp.full_name}>{emp.full_name} (ID: {emp.id})</label>
                                    <input type="number" id={`hours_emp_${emp.id}`} value={employeeHoursInput[emp.id.toString()] || ''} onChange={(e) => handleHourInputChange(emp.id, e.target.value)} min="0" step="0.01" placeholder={`Horas (Acum: ${emp.accumulated_hours || 0})`} className="input-style text-xs py-1"/>
                                </div>
                            ))}
                        </div>
                    )}
                    {formConfirmError && <p className="text-red-500 text-xs p-2 bg-red-50 rounded">{formConfirmError}</p>}
                    <div className="pt-4 flex justify-end space-x-2 border-t"><button type="button" onClick={()=>setIsConfirmModalOpen(false)} className="btn-secondary-xs px-3 py-1.5">Cancelar</button><button type="submit" disabled={isSubmittingConfirm || isLoadingConfirmData} className="btn-success-xs px-3 py-1.5 disabled:opacity-50">{isSubmittingConfirm ? 'Confirmando...' : 'Confirmar y Aplicar Nómina'}</button></div>
                </form>
            </Modal>
            )}
            ...
            <Modal isOpen={isStatusModalOpen} onClose={() => setIsStatusModalOpen(false)} title={`Actualizar Estado de Nómina: "${runToUpdateStatus.name}"`}>
                <form onSubmit={handleSubmitStatusUpdate} className="space-y-4 text-sm">
                    ...
                    <div className="pt-4 flex justify-end space-x-2 border-t"><button type="button" onClick={()=>setIsStatusModalOpen(false)} className="btn-secondary-xs px-3 py-1.5">Cancelar</button><button type="submit" disabled={isSubmittingStatusUpdate} className="btn-primary-xs px-3 py-1.5 disabled:opacity-50">{isSubmittingStatusUpdate ? 'Actualizando...' : 'Actualizar Estado'}</button></div>
                </form>
            </Modal>
            )}
        </div>
